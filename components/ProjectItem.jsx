  ...
  return (
    <div key={project.id} className="border border-gray-300 rounded-md shadow-lg group">
        <div className='relative'>
            <img className='relative rounded-md w-full h-auto aspect-[3/2] object-fill' src={project.image} alt={project.title}/>
            <div className='hidden group-hover:block ease-in-out cursor-pointer'>
                <div className='absolute left-0 top-0 bg-black opacity-50 h-full w-full' />
                <div className='absolute left-[35%] top-[80%] translate-x-[-50%] translate-y-[-50%] text-white'>
                    <p className='text-l font-semibold tracking-wider'>{project.title}</p>
                    <p className='text-sm mt-2 max-h-[60%] overflow-hidden'>{project.description}</p>
                </div>
